...
function newModel(type: 'CHAT' | 'IMAGE'): ProviderModelRequest {
  return {
    ...
    timeoutSeconds: type === 'CHAT' ? 600 : 900,
    maxRetries: type === 'IMAGE' ? 2 : null,
    retryBackoffSeconds: type === 'IMAGE' ? 3 : null,
...
function open(provider?: ProviderAccount | null) {
  const source = provider || props.provider || null
  ...
  form.value = source
    ? {
        ...
        models: source.models.map((model) => ({
          ...
          timeoutSeconds: model.timeoutSeconds ?? (model.type === 'CHAT' ? 600 : 900),
          maxRetries: model.maxRetries ?? (model.type === 'IMAGE' ? 2 : null),
          retryBackoffSeconds: model.retryBackoffSeconds ?? (model.type === 'IMAGE' ? 3 : null),
  ...
  if (source?.providerKey.toLowerCase() === 'grsai') void loadGrsaiCatalog(false)
}
...
function availableModelOptions(model: ProviderModelRequest): string[] {
  const selectedByOtherRows = new Set(
    form.value.models
      .filter((item) => item !== model && item.type === model.type)
  ...
  >
    <el-form label-width="118px" class="provider-form">
      ...
      <el-form-item label="供应商标识" required>
        <el-input v-model="form.providerKey" placeholder="例如 openai、company-gateway" />
      </el-form-item>
      ...
      <el-alert
        ...
        :closable="false"
        show-icon
        title="Grsai 已内置默认基础地址和图片模型目录；填写 API Key 后即可使用，基础地址仍可按需修改。"
      />
      ...
      <div v-for="(model, index) in form.models" :key="model.id ?? `new-${index}`" class="model-card">
        ...
        <div class="model-grid">
          ...
          <el-form-item v-if="model.type === 'IMAGE'" label="失败重试">
...
<style scoped>
.provider-form { max-height:72vh; padding-right:8px; overflow-y:auto; }.inline-field { display:flex; width:100%; gap:8px; }.inline-field .el-input { flex:1; }.inline-field :deep(.el-button) { border-radius:8px; }.stacked-field { display:flex; flex-direction:column; width:100%; gap:8px; }.model-actions { display:flex; align-items:center; justify-content:space-between; margin-bottom:14px; color:#6d7891; font-size:13px; }.model-card { padding:16px 16px 6px; margin-bottom:16px; border:1px solid #e5e9f5; border-radius:12px; background:linear-gradient(145deg,#fbfcff,#f7f8fd); }.model-card-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:14px; }.model-title { display:flex; align-items:center; gap:10px; color:#44516d; }.model-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); column-gap:18px; }.model-grid :deep(.el-select),.model-grid :deep(.el-input-number) { width:100%; }.retry-field { display:flex; align-items:center; gap:6px; color:#8d97ac; font-size:12px; }.catalog-meta { display:flex; flex-wrap:wrap; align-items:center; gap:6px; color:#65718a; font-size:12px; }.retry-field :deep(.el-input-number) { width:100px; } @media(max-width:800px){.model-grid{grid-template-columns:1fr;}.model-actions{align-items:flex-start;gap:10px;flex-direction:column;}.inline-field{flex-wrap:wrap;}.inline-field .el-input{flex-basis:100%;}}
</style>
